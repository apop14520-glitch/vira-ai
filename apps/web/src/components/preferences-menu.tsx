...
import { AdminSession, changePassword, getSession, logout } from "@/lib/auth-api";
...
export function PreferencesMenu() {
  ...
  return (
    <div ref={menuRef} className="relative">
      ...
      {open && <div role="dialog" aria-label="Configurações do VIRA.AI" className="settings-panel absolute right-0 top-12 z-50 grid w-[min(94vw,680px)] grid-cols-[128px_minmax(0,1fr)] overflow-hidden rounded-2xl border border-slate-300 bg-white text-slate-950 shadow-2xl shadow-slate-950/25 dark:border-slate-700 dark:bg-slate-900 dark:text-white sm:grid-cols-[190px_minmax(0,1fr)]">
...
function SecuritySection({ session, currentPassword, newPassword, confirmation, setCurrentPassword, setNewPassword, setConfirmation, saving, message, submit, signOut }: { session: AdminSession | null; currentPassword: string; newPassword: string; confirmation: string; setCurrentPassword: (value: string) => void; setNewPassword: (value: string) => void; setConfirmation: (value: string) => void; saving: boolean; message: string | null; submit: (event: FormEvent<HTMLFormElement>) => void; signOut: () => void }) {
  return <div className="space-y-4"><SettingCard title="Sessão atual" description="A sessão é protegida por cookie e expira automaticamente após oito horas."><div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3.5 py-3 dark:border-slate-800 dark:bg-slate-900"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Administrador</p><p className="mt-1 text-sm font-semibold">{session?.username ?? "Sessão não identificada"}</p></div><button type="button" onClick={() => void signOut()} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-800 transition hover:border-rose-300 hover:bg-rose-50 hover:text-rose-800 dark:border-slate-700 dark:text-slate-200 dark:hover:border-rose-400/50 dark:hover:bg-rose-400/10 dark:hover:text-rose-200">Sair</button></div></SettingCard><SettingCard title="Alterar senha" description="Use uma senha com pelo menos 12 caracteres. A troca encerra todas as sessões ativas."><form onSubmit={submit} className="space-y-3"><PasswordField id="current-password" label="Senha atual" value={currentPassword} onChange={setCurrentPassword} autoComplete="current-password" /><PasswordField id="new-password" label="Nova senha" value={newPassword} onChange={setNewPassword} autoComplete="new-password" /><PasswordField id="password-confirmation" label="Confirmar nova senha" value={confirmation} onChange={setConfirmation} autoComplete="new-password" /><button type="submit" disabled={saving || !currentPassword || !newPassword || !confirmation} className="w-full rounded-xl bg-cyan-500 px-3 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-50">{saving ? "Alterando…" : "Alterar senha"}</button></form>{message && <p role="status" aria-live="polite" className="mt-3 text-sm font-medium leading-6 text-slate-700 dark:text-slate-300">{message}</p>}</SettingCard><SettingCard title="Controles ativos" description="Proteções aplicadas desde a fundação técnica."><StatusRow label="Sessões opacas e HttpOnly" value="Ativo" /><StatusRow label="Menor privilégio" value="Obrigatório" /><StatusRow label="Auditoria sem segredos" value="Ativa" /><StatusRow label="Dados pessoais no Business" value="Não coletados" /></SettingCard></div>;
}

function PasswordField({ id, label, value, onChange, autoComplete }: { id: string; label: string; value: string; onChange: (value: string) => void; autoComplete: string }) { return <label htmlFor={id} className="block text-sm font-semibold">{label}<input id={id} type="password" value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autoComplete} required className="mt-2 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-3 text-base text-slate-950 outline-none placeholder:text-slate-500 focus:border-cyan-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label>; }
