...
A API permanece no Railway. O frontend usa somente a variável server-side
`API_INTERNAL_URL`, com a origem pública padrão declarada no `wrangler.jsonc`;
nenhum token, senha ou chave de integração deve ser versionado.
